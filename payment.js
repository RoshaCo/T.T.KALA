/* =========================================================
   ...
   روند:
   انتخاب محصول ← ایجاد سفارش ← پرداخت زرین‌پال
...
(function () {
    ...
    if (purchaseProductSummary) {
      purchaseProductSummary.innerHTML = `
        ...
        <div class="purchase-product-chance">
          با این خرید، یک شانس برای دوره جاری
          برای شما ثبت می‌شود.
        </div>
      `;
    }
  ...
  function showPaymentSuccess(order) {
    ...
    /*
     * ذخیره موقت شناسه سفارش فقط برای ادامه روند
     * بعد از پرداخت.
     */
  ...
  async function startPurchase() {
    ...
    if (!productId) {
      showToast(
        "لطفاً ابتدا یک محصول انتخاب کنید."
      );
      return;
    }
    ...
    try {
      ...
      /*
       * انتقال مستقیم به صفحه امن پرداخت زرین‌پال
       */
  ...
  async function handlePaymentReturn() {
    ...
    try {
      /*
       * تأیید نهایی همیشه توسط سرور انجام می‌شود.
       * نتیجه URL به تنهایی قابل اعتماد نیست.
       */
      ...
      if (
        ...
        /*
         * حذف پارامترهای درگاه از آدرس مرورگر
         */
  ...
  async function registerPurchaseMobile() {
    ...
    try {
       ...
       * سرور بررسی می‌کند:
       *
       * - پرداخت واقعاً موفق بوده باشد
       * - سفارش متعلق به دوره جاری باشد
       * - قبلاً برای این سفارش شماره ثبت نشده باشد
       * - شماره معتبر باشد
       */
      ...
      if (
        ...
        /*
         * پس از ثبت موفق شماره، امکان دانلود فعال می‌شود.
         */

        await loadDownloadLink();

        /*
         * شماره دیگر قابل ثبت مجدد برای همین سفارش نیست.
         */
